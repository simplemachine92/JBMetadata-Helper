...
function getMetadata(id: string, metadata: string): [boolean, string] {
  const metadataBytes = ethers.utils.arrayify(metadata);

  if (metadataBytes.length <= MIN_METADATA_LENGTH) return [false, ""];

  const firstOffset = metadataBytes[RESERVED_SIZE + ID_SIZE];

  for (
    let i = RESERVED_SIZE;
    metadataBytes[i + ID_SIZE] != 0 && i < firstOffset * WORD_SIZE;

  ) {
    const currentOffset = metadataBytes[i + ID_SIZE];
    const idBytes = metadataBytes.slice(i, i + ID_SIZE);
    const idHex = ethers.utils.hexlify(idBytes);

    if (idHex == id) {
      const end =
        i + NEXT_DELEGATE_OFFSET >= firstOffset * WORD_SIZE ||
        metadataBytes[i + NEXT_DELEGATE_OFFSET] == 0
          ? metadataBytes.length
          : metadataBytes[i + NEXT_DELEGATE_OFFSET] * WORD_SIZE;

      const targetMetadataBytes = metadataBytes.slice(
        currentOffset * WORD_SIZE,
        end
      );
      const targetMetadata = ethers.utils.hexlify(targetMetadataBytes);
